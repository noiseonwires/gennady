...
func (b *Bot) handleMessage(message *tgbotapi.Message, isEdited bool) {
	...
	if b.config.IsModerationChat(message.Chat.ID) {
		...
		if isExcludedFromAnalysis {
			log.Printf("Skipping content analysis for message %d - reply to excluded message ID", message.MessageID)
		} else {
			...
			if b.config.AI.Enabled && !skipVision && (b.config.AI.ContentModeration.VisionEnabled || b.config.AI.ContentModeration.OCRSpaceEnabled) {
			...
			if contentSafetyFlagged {
				...
				// analyzeMessage bundles the deletion-queue insert into the same
				// transaction as message_info + profile tracking for non-edited
				// messages, so we mark the deletion as handled here.
			...
			if b.config.AI.Enabled && b.config.AI.MessageSummaries.Enabled {
				...
				if !isEdited && utf8.RuneCountInString(enhancedText) > b.config.AI.MessageSummaries.MinLength && !isExcludedFromSummary && !isExcludedUser {
					...
					log.Printf("Skipping AI summary for message %d - reply to excluded message ID", message.MessageID)
